...
class StatsManager:
    ...
    def _load(self) -> None:
        try:
            ...
            logger.info(
                "Stats loaded: vanilla=%s limitless=%s",
                self.vanilla is not None,
                self.limitless is not None,
            )
...
def api_basic_vehicle(vehicle_id: int, mode: str = "vanilla"):
    ...
    try:
        ...
        norm = stats.normalise_stats(
            v_stats=v_stats,
            c_stats=stats.EMPTY_DICT(),
            vehicles=stats_data["vehicles"],
            characters=[],
        )
        return {"stats": norm}
...
def api_basic_character(character_id: int, mode: str = "vanilla"):
    ...
    try:
        ...
        norm = stats.normalise_stats(
            v_stats=stats.EMPTY_DICT(),
            c_stats=c_stats,
            vehicles=[],
            characters=stats_data["characters"],
        )
        return {"stats": norm}
    except StopIteration:
...
def api_simulate(data: Dict[str, Any]):
    ...
    try:
        ...
        # Look up combo1
        vehicle1 = next(
            v for v in stats_data["vehicles"] if v.id == combo1.get("vehicle_id")
        )
        character1 = next(
            c for c in stats_data["characters"] if c.id == combo1.get("character_id")
        )
        ...
        if (
            combo2
            and combo2.get("vehicle_id") is not None
            and combo2.get("character_id") is not None
        ):
            wheelie2 = bool(combo2.get("wheelie", False))
            ssmt2 = bool(combo2.get("ssmt", False))
            smt2 = bool(combo2.get("smt", False))
            vehicle2 = next(
                v for v in stats_data["vehicles"] if v.id == combo2.get("vehicle_id")
            )
            character2 = next(
                c
                for c in stats_data["characters"]
                if c.id == combo2.get("character_id")
            )

        # Run simulations
        if sim_type == "accel":
            result1 = simulation.simulate_accel(
                vehicle1.get_basic_stats(),
                character1.get_basic_stats(),
                wheelie=wheelie1,
                ssmt=ssmt1,
                time=time,
            )
            if vehicle2 is not None:
                result2 = simulation.simulate_accel(
                    vehicle2.get_basic_stats(),
                    character2.get_basic_stats(),
                    wheelie=wheelie2,
                    ssmt=ssmt2,
                    time=time,
                )
            ...
            result1 = simulation.simulate_mini_turbo(
                vehicle1.get_basic_stats(),
                character1.get_basic_stats(),
                wheelie=wheelie1,
                SMT=smt1,
            ...
            if vehicle2 is not None:
                result2 = simulation.simulate_mini_turbo(
                    vehicle2.get_basic_stats(),
                    character2.get_basic_stats(),
                    wheelie=wheelie2,
                    SMT=smt2,
